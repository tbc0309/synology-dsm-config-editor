...
if [ "$ACTION" = "status" ]; then
    ...
    [ -x "$lifecycle_script" ] || json_error '500 Internal Server Error' '无法执行 start-stop-status'
    service_output=$("$lifecycle_script" status 2>&1)
    service_code=$?
    if [ "$service_code" -eq 0 ]; then
        case "$service_output" in *' is not running'*) service_code=3 ;; esac
    fi
    case "$service_code" in
        0) service_state='running' ;; 1) service_state='deadPid' ;;
        2) service_state='deadLock' ;; 3) service_state='stopped' ;;
        150) service_state='broken' ;; *) service_state='unknown' ;;
    esac
    printf 'Content-Type: application/json; charset=utf-8\r\nCache-Control: no-store\r\n\r\n'
    printf '{"ok":true,"running":%s,"state":"%s","code":%s}\n' \
        "$([ "$service_code" -eq 0 ] && printf true || printf false)" "$service_state" "$service_code"
    exit 0
fi
...
<main class="app">
  <header>
    <h1><span class="dot"></span><span>配置文件编辑器</span></h1>
    <div class="head-meta"><button class="service-state unknown" id="serviceStatus" title="点击重新检测">检测状态…</button><span class="badge" id="formatBadge">CONFIG</span><span id="stats">准备读取…</span></div>
  </header>
  <section class="toolbar">
    ...
    <span class="service-tools" id="serviceTools" hidden>
      <label class="hint" for="port">服务端口</label>
      <input id="port" type="text" readonly aria-readonly="true">
      <button class="open" id="open">↗ <span>打开服务</span></button>
    </span>
  </section>
  <section class="editor">
    ...
    <div class="code-pane">
      ...
      <textarea id="text" wrap="off" spellcheck="false" aria-label="配置文件内容"></textarea>
    </div>
  </section>
  <footer class="status"><span id="message">正在读取配置文件…</span><span id="cursor">行 1，列 1</span></footer>
</main>
<script>
(() => {
  'use strict';
  ...
  let baseline = '', busy = false, language = 'generic', openPath = '/';
  const messages = {
    save:'✓ 保存配置',saveChanges:'● 保存更改',reload:'↻ 重新读取',
    loading:'正在读取配置文件…',loaded:'已读取最新内容',
    unsavedReload:'尚有未保存的更改，确定重新读取吗？',saving:'正在保存并创建备份…',
    invalidPort:'配置的端口无效',lineColumn:'行 {line}，列 {column}',
    stats:'{lines} 行 · {bytes} 字节',
    saved:'配置保存成功，已创建备份',restarted:'配置已保存，套件已重新启动',
    lifecycleMissing:'配置已保存，但找不到 start-stop-status',
    lifecycleNotExecutable:'配置已保存，但 start-stop-status 没有执行权限',
    stopFailed:'配置已保存，但套件停止失败',startFailed:'配置已保存，但套件启动失败',
    scriptPathInvalid:'配置已保存，但重启脚本路径无效',scriptMissing:'配置已保存，但找不到重启脚本',
    scriptNotExecutable:'配置已保存，但重启脚本没有执行权限',
    scriptSucceeded:'配置已保存，重启脚本执行成功',scriptFailed:'配置已保存，但重启脚本执行失败'
  };
  const t = (key, vars={}) => {
  ...
  async function refreshServiceStatus(attempts=1) {
    ...
    badge.textContent='检测状态…';
    badge.className='service-state unknown';
    let data=null;
    for(let attempt=0;attempt<attempts;attempt++) {
      try { data=await fetch(endpoint('status'),{cache:'no-store'}).then(x=>x.json()); if(!data.ok)throw new Error() }
      catch(_){data=null}
      if(data?.running||attempt===attempts-1)break;
      await new Promise(resolve=>setTimeout(resolve,600));
    }
    const labels={running:'运行中',stopped:'已停止',deadPid:'进程异常',deadLock:'锁定异常',broken:'套件损坏',unknown:'状态未知'};
    badge.textContent=data?(labels[data.state]||'状态未知'):'状态未知';
  ...
  async function persist() {
    ...
    try {
      ...
      if(data.restartOk!==false&&service&&!service.running)setMessage('配置已保存，但服务未能正常运行','error');
      else setMessage(data.messageCode?t(data.messageCode):data.message,data.restartOk===false?'error':'ok');
    } catch(e) { setMessage(e.message,'error'); } finally { setBusy(false); }
  }
